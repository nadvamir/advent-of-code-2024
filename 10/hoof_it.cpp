...
using Peaks = std::optional<std::set<std::pair<int64_t, int64_t>>>;

Peaks map_trails(const auto& input, auto i, auto j, auto& reachability) {
    if (reachability[i][j]) return reachability[i][j];
    if (input[i][j] == '9') {
        reachability[i][j] = Peaks{{{i, j}}};
    ...
    reachability[i][j] = std::set<std::pair<int64_t, int64_t>>{};
    ...
    for (auto [di, dj] : directions) {
        ...
        auto reachable = map_trails(input, ni, nj, reachability);
        if (reachable) {
            reachability[i][j]->insert(begin(*reachable), end(*reachable));
        }
    }
    return reachability[i][j];
}
...
int64_t solve(const std::vector<std::string>& input) {
    std::vector<std::vector<Peaks>> reachability(input.size(), std::vector(input[0].size(), Peaks{}));
    int64_t result = 0;
    for (int64_t i = 0; i < input.size(); ++i) {
        for (int64_t j = 0; j < input[i].size(); ++j) {
            if (input[i][j] == '0') {
                result += map_trails(input, i, j, reachability)->size();
            }
        }
    }
    return result;
